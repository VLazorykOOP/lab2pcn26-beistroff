...
int main() {
    ...
    const char* fileName = "test_code.cpp"; // Файл C++

    // Створюємо справжній файл з кодом С++ для тесту
    ofstream testFile(fileName, ios::trunc);
    testFile << "#include <iostream>\n\n";
    testFile << "int main() {\n";
    testFile << "    std::cout << \"Привіт! Цей код був переданий через сокети фрагментами.\\n\";\n";
    testFile << "    return 0;\n";
    testFile << "}\n";
    testFile.close();
    ...
    if (inFile.read(fileData.data(), fileSize)) {
        cout << "Передача файлу '*.cpp' у " << XX << " фрагментах...\n";
